    ...
    for log in page_obj:
        ...
        if log.action == 'CREATE':
            ...
            if log.new_values:
                ...
                fields = []
                for key in list(log.new_values.keys())[:3]:
            ...
            if changes:
                changes_summary = "Изменения: " + ", ".join(changes[:3])
                if len(changes) > 3:
...
def export_students_excel(request):
    ...
    for row_num, student in enumerate(students_qs, 2):
        ...
        ws.cell(row=row_num, column=5).value = student.user.username
        ws.cell(row=row_num, column=6).value = student.user.email or ''
        ws.cell(row=row_num, column=7).value = student.phone or ''
        ws.cell(row=row_num, column=8).value = student.course
        ws.cell(row=row_num, column=9).value = student.student_group.name if student.student_group else 'Без класса'
        ws.cell(row=row_num, column=10).value = student.birth_date.strftime('%d.%m.%Y') if student.birth_date else ''
        ws.cell(row=row_num, column=11).value = student.address or ''
        ws.cell(row=row_num, column=12).value = 'Активен' if student.user.is_active else 'Заблокирован'
        ws.cell(row=row_num, column=13).value = student.user.date_joined.strftime('%d.%m.%Y %H:%M')
        ws.cell(row=row_num, column=14).value = student.user.last_login.strftime('%d.%m.%Y %H:%M') if student.user.last_login else 'Никогда'
...
def export_groups_excel(request):
    ...
    summary_sheet = wb.create_sheet("Сводка по классам", 0)
